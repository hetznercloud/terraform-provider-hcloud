...
// DataSourceListType is the type name of the Hetzner Storage Box Subaccounts data source.
const DataSourceListType = "hcloud_storage_box_subaccounts"
...
var _ util.ModelFromAPI[[]*hcloud.StorageBoxSubaccount] = &dataSourceListModel{}

func (m *dataSourceListModel) FromAPI(ctx context.Context, in []*hcloud.StorageBoxSubaccount) diag.Diagnostics {
	...
	m.Subaccounts, newDiags = types.ListValue((&model{}).tfType(), tfItems)
	diags.Append(newDiags...)
...
func (d *DataSourceList) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	result, err = d.client.StorageBox.AllSubaccountsWithOpts(ctx, storageBox, opts)
	if err != nil {
