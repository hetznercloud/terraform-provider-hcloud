...
func TestCertificateResource(t *testing.T) {
	t.Run("uploaded", func(t *testing.T) {
		t.Run("basic", func(t *testing.T) {
			...
			res := NewUploadedRData(t, "basic-cert", "TFAccTests")
			resRenamed := &RDataUploaded{Name: res.Name + "-renamed", PrivateKey: res.PrivateKey, Certificate: res.Certificate}
			...
			resource.Test(t, resource.TestCase{
				...
				CheckDestroy:             testsupport.CheckResourcesDestroyed(certificate.UploadedResourceType, ByID(t, &cert)),
		...
		t.Run("change-cert-requires-new-resource", func(t *testing.T) {
			...
			resOtherCert := &RDataUploaded{Name: res.Name, PrivateKey: rKey, Certificate: rCert}
				...
				Steps: []resource.TestStep{
					...
					},
					{
						...
						Check: resource.ComposeAggregateTestCheckFunc(
							...
							testsupport.CheckResourceExists(res.TFID(), ByID(t, &newCert)),
	...
	t.Run("managed", func(t *testing.T) {
		t.Run("basic", func(t *testing.T) {
			...
			res := NewManagedRData(t, "basic-managed-cert", []string{
				fmt.Sprintf("tftest-%d.%s", acctest.RandInt(), certDomain),
			})
			resRenamed := &RDataManaged{
			...
			resource.Test(t, resource.TestCase{
				PreCheck:                 teste2e.PreCheck(t),
				ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
				CheckDestroy:             testsupport.CheckResourcesDestroyed(certificate.ManagedResourceType, ByID(t, &cert)),
				Steps: []resource.TestStep{
					{
						// Create a new Certificate using the required values
						// only.
