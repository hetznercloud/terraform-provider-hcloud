...
import (
	...
	"github.com/hetznercloud/terraform-provider-hcloud/test/e2e/firewall"
	"github.com/hetznercloud/terraform-provider-hcloud/test/e2e/image"
	"github.com/hetznercloud/terraform-provider-hcloud/test/e2e/network"
	"github.com/hetznercloud/terraform-provider-hcloud/test/e2e/placementgroup"
	"github.com/hetznercloud/terraform-provider-hcloud/test/e2e/primaryip"
...
func TestServerResource(t *testing.T) {
	t.Run("basic", func(t *testing.T) {
		...
		resRenamed := &RData{Name: res.Name + "-renamed", Type: res.Type, Image: res.Image}
		resRenamed.SetRName(res.Name)
		tmplMan := testtemplate.Manager{}
		resource.ParallelTest(t, resource.TestCase{
			PreCheck:                 teste2e.PreCheck(t),
			ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
			CheckDestroy:             testsupport.CheckResourcesDestroyed(server.ResourceType, ByID(t, &s)),
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(res.TFID(), ByID(t, &s)),
	...
	t.Run("change-user-data", func(t *testing.T) {
		...
		resChangedUserdata := &RData{Name: res.Name, Type: res.Type, Image: res.Image, UserData: "updated stuff"}
		...
		resource.ParallelTest(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				},
				{
					...
					Check: resource.ComposeAggregateTestCheckFunc(
						testsupport.CheckResourceExists(res.TFID(), ByID(t, &s2)),
	...
	t.Run("direct-attach-to-network", func(t *testing.T) {
		var (
			nw  hcloud.Network
			nw2 hcloud.Network
			s   hcloud.Server
			...
			updateIP = func(d *RData, networkID string, ip string) *RData {
			...
			updateAliasIPs = func(d *RData, networkID string, ips ...string) *RData {
				for i := range d.Networks {
					if d.Networks[i].NetworkID == networkID {
			...
			addNetwork = func(d *RData, network RDataInlineNetwork) *RData {
		...
		resource.ParallelTest(t, resource.TestCase{
			PreCheck:                 teste2e.PreCheck(t),
			ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
			CheckDestroy:             testsupport.CheckResourcesDestroyed(server.ResourceType, ByID(t, nil)),
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						testsupport.CheckResourceExists(sResWithNet.TFID(), ByID(t, &s)),
				...
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(sRes.TFID(), ByID(t, &s)),
				...
				},
				{
					...
					Config: tmplMan.Render(t,
						...
						"testdata/r/hcloud_network", nwRes,
						"testdata/r/hcloud_network_subnet", snwRes,
						"testdata/r/hcloud_server", addNetwork(sResWithNet, RDataInlineNetwork{
							NetworkID: nwRes.TFID() + ".id",
							IP:        "10.0.1.8",
						}),
					),
	...
	t.Run("primary-ip-network", func(t *testing.T) {
		var (
			nw hcloud.Network
			s  hcloud.Server
			p  hcloud.PrimaryIP
		...
		sResWithOnlyIPv6AutoGenerated := &RData{
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(nwRes.TFID(), network.ByID(t, &nw)),
						testsupport.CheckResourceExists(sResWithNetAndPublicNet.TFID(), ByID(t, &s)),
						testsupport.LiftTCF(hasServerNetwork(t, &s, &nw, "10.0.1.5", "10.0.1.6", "10.0.1.7")),
				...
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(nwRes.TFID(), network.ByID(t, &nw)),
						testsupport.CheckResourceExists(sResWithoutPublicNet.TFID(), ByID(t, &s)),
						testsupport.LiftTCF(hasServerNetwork(t, &s, &nw, "10.0.1.5", "10.0.1.6", "10.0.1.7")),
				...
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						testsupport.CheckResourceExists(sResWithPrimaryIP.TFID(), ByID(t, &s)),
						testsupport.LiftTCF(func() error {
				...
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						testsupport.CheckResourceExists(sResWithOnlyIPv6.TFID(), ByID(t, &s)),
						testsupport.LiftTCF(func() error {
				...
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(primaryIPv6Res.TFID(), primaryip.ByID(t, &p)),
						testsupport.CheckResourceExists(sResWithOnlyIPv6AutoGenerated.TFID(), ByID(t, &s)),
						testsupport.LiftTCF(func() error {
							assert.NotEqual(t, p.ID, s.PublicNet.IPv4.ID)
							assert.Equal(t, 0, s.PublicNet.IPv4.ID)
							assert.NotEqual(t, 0, s.PublicNet.IPv6.ID)
	...
	t.Run("private-network-bastion", func(t *testing.T) {
...
`, sshKeyRes.PrivateKey),
	...
	t.Run("placement-group", func(t *testing.T) {
		var (
		...
		resource.ParallelTest(t, resource.TestCase{
			PreCheck:                 teste2e.PreCheck(t),
			ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
			CheckDestroy:             testsupport.CheckResourcesDestroyed(server.ResourceType, ByID(t, &srv)),
			Steps: []resource.TestStep{
				{
					// Create a new Server using the required values
					// only.
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(srvRes.TFID(), ByID(t, &srv)),
