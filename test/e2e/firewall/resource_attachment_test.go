...
func TestFirewallResourceAttachment(t *testing.T) {
	t.Run("servers", func(t *testing.T) {
		...
		fwRes := NewRData(t, "basic_firewall", nil, nil)
		srvRes := &server2.RData{
			Name:  "test-server",
		...
		fwAttRes := NewRDataAttachment("fw_ref", fwRes.TFID()+".id")
		...
		resource.Test(t, resource.TestCase{
			...
			CheckDestroy: resource.ComposeAggregateTestCheckFunc(
				...
				testsupport.CheckResourcesDestroyed(firewall.ResourceType, ByID(t, &fw)),
			),
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(srvRes.TFID(), server2.ByID(t, &srv)),
						testsupport.CheckResourceExists(fwRes.TFID(), ByID(t, &fw)),
	...
	t.Run("label-selectors", func(t *testing.T) {
		var (
			srv hcloud.Server
			fw  hcloud.Firewall
		)
