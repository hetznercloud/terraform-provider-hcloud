...
func TestFirewallResource(t *testing.T) {
	t.Run("basic", func(t *testing.T) {
		var f hcloud.Firewall

		res := NewRData(t, "basic-firewall", []RDataRule{
			{
				Direction:   "in",
				Protocol:    "tcp",
		...
		updated := NewRData(t, "basic-firewall", []RDataRule{
			{
				Direction:   "in",
				Protocol:    "tcp",
				SourceIPs:   []string{"0.0.0.0/0", "::/0"},
		...
		resource.Test(t, resource.TestCase{
			...
			CheckDestroy:             testsupport.CheckResourcesDestroyed(firewall.ResourceType, ByID(t, &f)),
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(res.TFID(), ByID(t, &f)),
	...
	t.Run("apply-to", func(t *testing.T) {
		...
		res := NewRData(t, "applyto-firewall", []RDataRule{
			{
				Direction:   "in",
				Protocol:    "tcp",
				SourceIPs:   []string{"0.0.0.0/0", "::/0"},
		...
		resUpdated := NewRData(t, "applyto-firewall", []RDataRule{
			{
				Direction:   "in",
				Protocol:    "tcp",
				SourceIPs:   []string{"0.0.0.0/0", "::/0"},
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				...
				},
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(resUpdated.TFID(), ByID(t, &f)),
	...
	t.Run("normalization", func(t *testing.T) {
		...
		res := NewRData(t, "ipv6-firewall", []RDataRule{
			{
				Direction: "in",
				Protocol:  "tcp",
				// Uppercase
