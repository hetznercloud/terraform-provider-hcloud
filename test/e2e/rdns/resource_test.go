...
import (
	...
	primaryip "github.com/hetznercloud/terraform-provider-hcloud/internal/primaryip"
	"github.com/hetznercloud/terraform-provider-hcloud/internal/server"
	"github.com/hetznercloud/terraform-provider-hcloud/internal/teste2e"
	"github.com/hetznercloud/terraform-provider-hcloud/internal/testsupport"
	"github.com/hetznercloud/terraform-provider-hcloud/internal/testtemplate"
	floatingiptest "github.com/hetznercloud/terraform-provider-hcloud/test/e2e/floatingip"
	loadbalancer2 "github.com/hetznercloud/terraform-provider-hcloud/test/e2e/loadbalancer"
	primaryiptest "github.com/hetznercloud/terraform-provider-hcloud/test/e2e/primaryip"
...
func TestRDNSResource(t *testing.T) {
	t.Run("server", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				resRDNS := NewRDataServer(t, tt.name, resServer.TFID()+".id", resServer.TFID()+tt.ipAddress, tt.dns)

				// TODO: Debug issues that causes this to fail when running in parallel
				resource.Test(t, resource.TestCase{
					PreCheck:                 teste2e.PreCheck(t),
					ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
					CheckDestroy:             testsupport.CheckResourcesDestroyed(server.ResourceType, server2.ByID(t, &s)),
					Steps: []resource.TestStep{
						{
							...
							Check: resource.ComposeTestCheckFunc(
								testsupport.CheckResourceExists(resServer.TFID(), server2.ByID(t, &s)),
	...
	t.Run("primary-ip", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				tmplMan := testtemplate.Manager{}
				restPrimaryIP := &primaryiptest.RData{
					Name:         tt.name,
				...
				resRDNS := NewRDataPrimaryIP(t, tt.name, restPrimaryIP.TFID()+".id", restPrimaryIP.TFID()+".ip_address", tt.dns)

				// TODO: Debug issues that causes this to fail when running in parallel
				resource.Test(t, resource.TestCase{
					PreCheck:                 teste2e.PreCheck(t),
					ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
					CheckDestroy:             testsupport.CheckResourcesDestroyed(primaryip.ResourceType, primaryiptest.ByID(t, &primaryIP)),
					Steps: []resource.TestStep{
						{
							...
							Check: resource.ComposeTestCheckFunc(
								testsupport.CheckResourceExists(restPrimaryIP.TFID(), primaryiptest.ByID(t, &primaryIP)),
								resource.TestCheckResourceAttr(resRDNS.TFID(), "dns_ptr", tt.dns),
							),
						},
						{
	...
	t.Run("floating-ip", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				tmplMan := testtemplate.Manager{}
				restFloatingIP := &floatingiptest.RData{
					Name:             tt.name,
					Type:             tt.floatingIPType,
					HomeLocationName: teste2e.TestLocationName,
				}
				restFloatingIP.SetRName(tt.name)
				resRDNS := NewRDataFloatingIP(t, tt.name, restFloatingIP.TFID()+".id", restFloatingIP.TFID()+".ip_address", tt.dns)

				// TODO: Debug issues that causes this to fail when running in parallel
				resource.Test(t, resource.TestCase{
					PreCheck:                 teste2e.PreCheck(t),
					ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
					CheckDestroy:             testsupport.CheckResourcesDestroyed(floatingip.ResourceType, floatingiptest.ByID(t, &fl)),
					Steps: []resource.TestStep{
						{
							// Create a new SSH Key using the required values
							// only.
							...
							Check: resource.ComposeTestCheckFunc(
								testsupport.CheckResourceExists(restFloatingIP.TFID(), floatingiptest.ByID(t, &fl)),
								resource.TestCheckResourceAttr(resRDNS.TFID(), "dns_ptr", tt.dns),
							),
						},
						{
	...
	t.Run("load-balancer", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				tmplMan := testtemplate.Manager{}
				restLoadBalancer := &loadbalancer2.RData{
					Name:         tt.name,
					LocationName: teste2e.TestLocationName,
				}
				restLoadBalancer.SetRName(tt.name)

				resRDNS := NewRDataLoadBalancer(t, tt.name, restLoadBalancer.TFID()+".id", restLoadBalancer.TFID()+tt.ipAddress, tt.dns)

				// TODO: Debug issues that causes this to fail when running in parallel
				resource.Test(t, resource.TestCase{
					PreCheck:                 teste2e.PreCheck(t),
					ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
					CheckDestroy:             testsupport.CheckResourcesDestroyed(loadbalancer.ResourceType, loadbalancer2.ByID(t, &lb)),
					Steps: []resource.TestStep{
						{
							Config: tmplMan.Render(t,
							...
							Check: resource.ComposeTestCheckFunc(
								testsupport.CheckResourceExists(restLoadBalancer.TFID(), loadbalancer2.ByID(t, &lb)),
								resource.TestCheckResourceAttr(resRDNS.TFID(), "dns_ptr", tt.dns),
							),
						},
						{
