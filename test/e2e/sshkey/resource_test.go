...
func TestSSHKeyResource(t *testing.T) {
	t.Run("basic", func(t *testing.T) {
		...
		res := NewRData(t, "basic-ssh-key")
		resRenamed := &RData{Name: res.Name + "-renamed", PublicKey: res.PublicKey}
		resRenamed.SetRName(res.Name)
		resource.ParallelTest(t, resource.TestCase{
			PreCheck:                 teste2e.PreCheck(t),
			ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
			CheckDestroy:             testsupport.CheckResourcesDestroyed(sshkey.ResourceType, ByID(t, &sk)),
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(res.TFID(), ByID(t, &sk)),
		...
		t.Run("upgrade-plugin-framework", func(t *testing.T) {
			...
			res := NewRData(t, "upgrade-plugin-framework-test")
