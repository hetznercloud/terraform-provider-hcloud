...
import (
	...
	"github.com/hetznercloud/terraform-provider-hcloud/test/e2e/server"
...
func TestFloatingIPResource(t *testing.T) {
	t.Run("basic", func(t *testing.T) {
		...
		resRenamed := &RData{Name: res.Name + "-renamed", Type: res.Type, HomeLocationName: res.HomeLocationName}
		resRenamed.SetRName(res.Name)
		tmplMan := testtemplate.Manager{}
		resource.ParallelTest(t, resource.TestCase{
			PreCheck:                 teste2e.PreCheck(t),
			ProtoV6ProviderFactories: teste2e.ProtoV6ProviderFactories(),
			CheckDestroy:             testsupport.CheckResourcesDestroyed(floatingip.ResourceType, ByID(t, &fip)),
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						testsupport.CheckResourceExists(res.TFID(), ByID(t, &fip)),
